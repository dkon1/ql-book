...
A \emph{first-order} ODE is one where the derivative $dx/dt$ is  equal to a *defining function* $f(x,t)$, like this: 
...
The constant of integration $C$ appears as in the standard anti-derivative definition. It can be specified by an initial condition for the solution $x(t)$.

The simplest method of analytic solution of a first-order ODEs, which I call \index{differential equation!separate-and-integrate} *separate-and-integrate* consists of the following steps:
...
### solutions of inhomogeneous ODEs
...
$$Ca e^{at} = aCe^{at} + aD +b$$
which is satisfied if $aD + b = 0$, which means $D= -b/a$. This is consistent with the example above, the additive constant in the solution was 0.8, which is $-b/a= -(4)/5=0.8$. 
