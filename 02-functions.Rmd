...
Let us now turn to more complex functions, made up of simpler components that we understand. Consider a ratio of two polynomials, called a \index{function!rational}\emph{rational function}. The general form of such functions can be written down as follows, where ellipsis stands for terms with powers lower than $n$ or $m$:
\begin{equation}
...
\textbf{Example.} The following model, called the \emph{Hill equation}\index{Hill equation}\index{model!Hill}, describes the fraction of receptor molecules which are bound to a ligand, which is a chemical term for a free molecule that binds to another, typically larger, receptor molecule. $\theta$ is the fraction of receptors bound to a ligand, $L$ denotes the ligand concentration, $K_d$ is the dissociation constant, and $n$ called the \emph{binding cooperativity}\index{binding cooperativity} or Hill coefficient:
...
\textbf{Example.} A common model of population over time is the \index{function!logistic}\emph{logistic function}. There are variations on how it is written down, but here is one general form:
\begin{equation}
