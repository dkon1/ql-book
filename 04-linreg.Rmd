...
Table: Body leanness (B) and heat loss rate (H)  in boys; partial dataset from \citep{sloan_cooling_1973}
...
summary(myfit)
m<-cov(data$HR,data$Temp)/var(my_data$Temp)
print(m)
b<-mean(my_data$HR)-m*mean(data$Temp)
print(b)
```
...
<<bio8-1, fig.width=4.5, fig.height=4.5, out.width='0.49\\linewidth',fig.show='hold',echo=FALSE>>=
library("HistData")
...
abline(0,1,lwd=3,lty=2,col='red')
 @
   \caption{Galton's data on heights of parents and of children as scatterplots (two versions with explanatory and response variables switched). The dotted red lines show the identity line $y=x$ and the solid black line is the linear regression. For the plot on the left, the best-fit slope is about 0.65, for the plot on the right, the slope is about 0.33.}
   \label{fig:regression2mean}
\end{figure}
...
\discussion{What types of mutations were observed in the data set? What were the most and the least common?}

\discussion{The paper shows that both maternal and paternal age are positively correlated with offspring inheriting new mutations. What biological mechanism explains why paternal age is the dominant factor? What could explain the substantial correlation with maternal age?}

\discussion{Is linear regression the best representation of the relationship between paternal age and number of mutations? What other model did the authors use to fit the data, and how did it perform?}

\discussion{What do you make of the historical data of paternal ages the authors present at the end of the paper? Can you postulate a testable hypothesis based on this observation?}

\discussionshere{Please read the paper on measuring the rate of \index{sequencing!whole genome}de novo mutations and its relationship to paternal age \citep{kong_rate_2012}.}

## Linear regression: parameters, plots, and residuals
